...
async function createTaskGuidance(req, res) {
  try {
    ...
    const generatedGuidance = await generateGuidance(
      projectDescription,
      taskTitle,
      taskDescription
    );
    ...
    const isValidGuidance =
      generatedGuidance &&
      Array.isArray(generatedGuidance.steps) &&
      generatedGuidance.steps.length > 0 &&
      typeof generatedGuidance.verification === "string";
